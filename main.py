...
military_collection = chroma_client.get_or_create_collection(name="military_knowledge")
...
def process_military_data():
    with open("military.json", "r", encoding="utf-8") as file:
        military_data = json.load(file)

    documents = []
    for item in military_data:
        documents.append(
            f"{item['title']} / 전화번호: {item['phoneNumber']} / 홈페이지: {item['page']}")
    return documents
        ...
        for item in student["student"]:
           ...
           # 전과
            ...
            if item["title"] == "제적 및 자퇴":
                ...
                text += f"신청 방법: {item.get('method')}\n"
                text += f"등록금 환불 주의 사항: {item.get('refund_note')}\n"
                text += f"등록금 환불 전화번호 문의: {item.get('refund_phoneNumber')}\n"
                text += f"전화번호 문의: {item.get('phoneNumber')}\n"
                text += f"홈페이지: {item.get('page')}\n"
...
def load_data_to_chroma():
    collections = {
        ...
        "military" : (military_collection, process_military_data()),
...
def ask():
    ...
    military_results = military_collection.query(query_embeddings=[user_embedding], n_results=counts)
    ...
    military_docs = [doc["text"] for doc in military_results["metadatas"][0] if "text" in doc]
    ...
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {"role": "system", "content": f"다음은 병무 관련 정보 입니다:\n\n{military}"},
